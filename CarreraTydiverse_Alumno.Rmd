...
```{r}
cran<-as_tibble(mydf) #Transormamos el dataframe original en un tibble.
rm(mydf) #Borramos el dataframe original.
...
pack_sum<- by_package %>% summarise(count = n(), unique = n_distinct(ip_id), countries = n_distinct(country), med_bytes = stats::median(size))
pack_sum
...
top_counts_sorted<- cran %>%  count(package) %>% filter(n > quantile(n, probs = 0.99)) %>% arrange(desc(n))
top_counts_sorted
...
top_unique_sorted <- cran %>%
  ...
  filter(unique > quantile(unique, 0.99)) %>%  
  arrange(desc(unique)) 
...
```{r}
#0.5 bytes son 500000 bytes
cran %>% filter(size<=500000)
...
```{r}
tidy_students2<-students2 %>% pivot_longer(cols = c(male_1, female_1, male_2, female_2),names_to = "class", values_to = "count") 
tidy_students2
...
student_info <- students4 %>% select(id, name, sex, class)%>% distinct() 


gradebook <- students4 %>% select(id, midterm, final) 
